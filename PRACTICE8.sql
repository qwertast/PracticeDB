...
WHERE customer_id IN (
    ...
    WHERE o.order_id IN (
        SELECT order_id
        FROM order_items
        WHERE price_per_unit = (SELECT MAX(price) FROM products)
    )
);
...
FROM Customers c1
LEFT JOIN Customers c2 ON c1.recommended_by = c2.customer_id
ORDER BY c2.full_name NULLS FIRST, c1.full_name;